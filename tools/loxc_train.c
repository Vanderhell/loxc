...
typedef struct {
  uint32_t symbol_id;
  uint64_t count;
  int is_dict;              /* 1 = dict entry, 0 = char */
...
static int analyze_freqs(const uint8_t *data, size_t data_len,
  ...
  printf("Dict entries analyzed: %zu total, %zu accepted (gain > 0), %zu rejected\n",
...
int main(int argc, char *argv[]) {
  ...
  if (result.strategy != LOXC_STRATEGY_FLAT_FIXED_WIDTH) {
    fprintf(stderr,
            "Error: code generation currently supports only FLAT_FIXED_WIDTH (strategy=%d)\n",
            (int)result.strategy);
    loxc_hier_free(&hier);
    free(freqs);
    free_symbol_recs(symbols, symbol_count);
