...
int main(void) {
  ...
  const char *text = "Hello from loxc examples";
  uint8_t encoded[8192];
  size_t encoded_cap = sizeof(encoded);
  size_t encoded_len = 0;
  assert(loxc_compress("demo", text, strlen(text), encoded, &encoded_cap,
                       &encoded_len) == LOXC_OK);
  ...
  printf("01_basic_compress: input=%zu encoded=%zu decoded=%zu\n",
